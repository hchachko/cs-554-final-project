...
router.patch("/profilePic", async (req, res) => {
  ...
  if (!getUserData.username && !getUserData.profilePic) {
    res
      .status(400)
      .json({ error: "You must supply a username and profile picture" });
    return;
  }
  let username = getUserData.username;
  ...
  if (!username || !password || !email) {
    res.status(400).json({
      error:
        "Error: You must provide a username, password, and email to create a new user.",
    });
  } else if (
    ...
    res.status(400).json({
      error: "Error: Username, password, and email must be valid strings.",
    });
  } else if (
    ...
    res.status(400).json({
      error: "Error: Username, password, and email must not be empty.",
    });
  }
  try {
